...
describe("onboarding state", () => {
  ...
  it("rebases parsed state to the requested user key", () => {
    const parsed = safeParseOnboardingState(
      JSON.stringify({
        userKey: "anonymous",
    ...
    expect(parsed.userKey).toBe("user-1");
    expect(parsed.completedSteps).toEqual(["signup"]);
  });

  it("keeps dismissed onboarding dismissed while background progress updates", () => {
    const dismissed = { ...createInitialOnboardingState("user-1"), dismissed: true };

    const withKey = markOnboardingStep(dismissed, "api-key", "complete");
    const resumed = resumeOnboarding(withKey);

    expect(withKey.dismissed).toBe(true);
    expect(buildOnboardingSteps(withKey).every((step) => step.state !== "current")).toBe(
      true,
    );
    expect(resumed.dismissed).toBe(false);
    expect(buildOnboardingSteps(resumed).some((step) => step.state === "current")).toBe(
      true,
    );
  });
