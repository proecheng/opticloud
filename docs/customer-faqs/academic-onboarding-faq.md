...
5-year reproducibility / image archival 指平台对已签发 durable voucher 的复现与镜像归档承诺。起算点是该 voucher 在 `reproduction_vouchers.created_at` 记录的 UTC 创建时间；rerun child voucher 使用 child voucher 自己的创建时间起算，不会延长 parent voucher 的 5 年时钟。

恢复操作以 [`docs/runbooks/repro-image-restore.md`](../runbooks/repro-image-restore.md) 为运营参考。完整 G7 分层归档 pipeline 上线前，对外仍应说明这是合同 / 路线图承诺，不要写成所有 archive restore 能力已经自助上线。
