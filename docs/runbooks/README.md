...
- 📄 **[`runbooks/repro-image-restore.md`](repro-image-restore.md)** ✅ **DRAFT READY**
- 🎯 内容：已签发 durable voucher 的 image restore 操作口径；5 年时钟从 `reproduction_vouchers.created_at` UTC 起算；记录恢复证据、异常和季度演练结果

#### 1.5 Voucher bitwise reproducibility audit SOP（季度抽样）

- 📄 **[`runbooks/repro-bitwise-audit.md`](repro-bitwise-audit.md)** ✅ **DRAFT READY**
- 🎯 内容：季度 5% issued voucher 抽样；当前 LP / `highs` live path 严格 digest 复现检查；输出 JSON / Markdown 审计报告，不创建 child voucher
