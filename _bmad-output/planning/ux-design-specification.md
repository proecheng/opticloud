...
```mermaid
graph TD
    A[Landing → 选'制造'] --> B[直接跳 Excel Upload Demo]
    B --> C[Console New Optimization]
    C --> D[Drag-drop .xlsx]
    D --> E{File 检测}
    E -->|密码保护 CS3.1| ER1[错误 + 移除密码引导]
    E -->|公式 CS3.2| ER2[Modal: 求值后导入 vs 保留公式]
    E -->|多 sheet CS3.3| ER3[Modal: 选 sheet name]
    ER1 --> D
    ER2 --> F
    ER3 --> F
    E --> F[自动 header 识别 + GBK/UTF-8]
    F --> G[预览前 5 行 + 老张 confirm header mapping]
    G --> H[选 SKU: 排程 / VRPTW / Job-Shop FG4 v2]
    H --> I[Credits 预检查 CST7]
    I --> J[Submit Solve]
    J --> K{Solve 1-90s<br/>长任务 SSE 进度 PM_26_12}
    K --> L[Result 三件套 同 J1]
    L --> M[下载结果 Excel 一键]
    M --> N[Excel 含:<br/>• 高亮 cells<br/>• Comment 解释 each cell<br/>• NL Summary tab]
    N --> O[老张 给老板看]
```
...
graph TD
    ...
    D -->|共享设备 TT5| F1[explicit check: 设备指纹相似 ≠ 同人]
    D -->|切换设备 TT6| F2[不触发 + 允许]
    D -->|VPN 同 /24 TT7| F3[不触发 + 允许]
    F1 --> H[允许或软提示]
    F2 --> H
    F3 --> H
    ...
    E2 --> G
    G --> H1{团队规模}
    H1 -->|标准档 ≥3 人| I[48h 人工复核]
    H1 -->|精简档 ≤2 人| J[Auto-score 复审]
    I --> K{结果}
    J --> K
    K -->|通过| L[账号解封]
    K -->|维持原判| M[账号合并提议]
...
graph TD
    ...
    E -->|普通用户 bug report TT8| F2[Bug report channel 独立]
    ...
    H -->|是| I[安全团队回邮 致谢 + 确认]
    I --> J{漏洞分级}
    J -->|CVSS ≥7.0| K1[24h Patch deploy]
    J -->|CVSS 4-6.9| K2[7day Patch deploy]
    K1 --> L[公开致谢页]
    K2 --> L
    L --> M{重复披露？CS5.2}
    M -->|第一披露| N[发放奖励 ¥500-2000]
    M -->|后续| O[致谢但不奖励]
```
