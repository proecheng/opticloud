...
test.describe.serial("老张 Excel vertical slice (3.E.9)", () => {
  test("老张从注册到下载库存结果 Excel 的完整路径", async ({ page }, testInfo) => {
    ...
    await test.step("老张 3/7 - 关闭欢迎页 modal 并进入 Excel surface", async () => {
      ...
      await expect(page.getByTestId("welcome-excel-upload-link")).toHaveAttribute(
        "href",
        "/console/excel",
      );
      await Promise.all([
        page.waitForURL(/\/console\/excel$/, { timeout: 10_000 }),
        page.getByTestId("welcome-excel-upload-link").click({ force: true }),
      ]);
    ...
    await test.step("老张 4/7 - 上传 Inventory workbook 并等待自然识别", async () => {
      ...
      await expect(modal).toContainText("系统判断：库存预测");
    ...
    await test.step("老张 5/7 - 确认识别结果并进入库存预览", async () => {
      ...
      await expect(preview).toContainText("已构建库存预测请求");
    ...
    await test.step("老张 6/7 - 试跑并确认 demo/501 诚实状态", async () => {
      ...
      await expect(demoCard).toContainText("当前版本返回演示结果");
    });
